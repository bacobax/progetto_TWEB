...
const getClubsByName = async (name) => {
    return axios.get(getJavaServerUrl(`/api/clubs/name/${name}`));
}
...
exports.search = catchAsync(async (req,res,next) =>{
    ...
    try{
        ...
        next(new AppError("Error while searching clubs: " + e.message, 500));
    }

    try{
        players = await  getPlayersByName(text);
    }catch(e){
        next(new AppError("Error while searching players: " + e.message, 500));
    }
